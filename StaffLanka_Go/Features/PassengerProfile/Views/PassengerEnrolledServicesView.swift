...
struct PassengerEnrolledServicesView: View {
    ...
    var body: some View {
        ...
        .confirmationDialog("Cancel this enrollment?", isPresented: $passengerEnrolledServiceViewModel.showCancelOptions, titleVisibility: .visible) {
            Button("Cancel Morning") { passengerEnrolledServiceViewModel.handleCancelChoice(.morning) }
            Button("Cancel Evening") { passengerEnrolledServiceViewModel.handleCancelChoice(.evening) }
            Button("Cancel Entire Service", role: .destructive) { passengerEnrolledServiceViewModel.handleCancelChoice(.both) }
            Button("Keep Service", role: .cancel) {}
        }
    }
    ...
    private var pastContent: some View {
        VStack(spacing: 16) {
            if passengerEnrolledServiceViewModel.pastServices.isEmpty {
                emptyState(
                ...
                ForEach(passengerEnrolledServiceViewModel.pastServices) { service in
                    pastServiceCard(service)
                }
            }
        }
        .padding(.horizontal, 16)
    ...
    private func activeServiceCard(_ service: EnrolledService) -> some View {
        VStack(spacing: 0) {
            ...
            if let evening = service.evening {
                sessionBlock(info: evening, label: "Evening", icon: "moon.stars.fill",
            ...
            HStack {
                VStack(alignment: .leading, spacing: 2) {
                    Text("Monthly Fee")
                        .font(.system(size: 12))
                        .foregroundColor(.textTertiary)
                    Text(String(format: "Rs. %.0f", service.monthlyFee))
    ...
    private func routeHeader(_ service: EnrolledService, dimmed: Bool) -> some View {
        HStack(spacing: 12) {
            ZStack {
                Circle()
                    .fill(Color.brandAccent.opacity(dimmed ? 0.07 : 0.12))
    ...
    private func sessionBlock(
        info: EnrolledSessionInfo,
        label: String,
        icon: String,
        iconColor: Color,
        service: EnrolledService,
        sessionType: EnrolledSessionType
    ) -> some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            VStack(spacing: 10) {
                ...
                infoRow(icon: info.vehicleType.lowercased() == "van" ? "car.fill" : "bus.fill",
            ...
            if service.session != .both {
                Button(role: .destructive) {
                    passengerEnrolledServiceViewModel.requestCancel(service: service, session: sessionType)
                } label: {
                    cancelLabel("Cancel \(label) Enrollment")
                }
                .buttonStyle(.plain)
            }
        }
    ...
    private func feeAndCancelFooter(_ service: EnrolledService) -> some View {
        VStack(spacing: 12) {
            HStack {
                VStack(alignment: .leading, spacing: 2) {
                    Text("Monthly Fee")
    ...
    private var hintSubtitle: String {
        if !passengerEnrolledServiceViewModel.hasMorningActive && !passengerEnrolledServiceViewModel.hasEveningActive { return "Browse routes to enrol in a morning or evening service." }
        if !passengerEnrolledServiceViewModel.hasMorningActive { return "You can enrol in a morning service from a different or the same driver." }
        return "You can enrol in an evening service from a different or the same driver."
    }
...
#Preview("Dark") {
    NavigationStack { PassengerEnrolledServicesView() }
